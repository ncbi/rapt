...
###############################* Global Constants *##################################
IMAGE_URI=us.gcr.io/ncbi-seqplus-rapt-build/rapt/rapt:develop-28112359
RAPT_VERSION=rapt-28112359
...
create_job()
{
	...
	local jobout="${dst_sto}/${job_id}.out"
...
YAML
	...
	env_vars=$(IFS=',';echo "${env_params[*]}")
	job_labels=$(IFS=',';echo "${labels[*]}")
	...
	gcloud beta lifesciences pipelines run \
		...
		--labels="${job_labels}" > "${TMP_DIR}/gcmsg" 2>&1
